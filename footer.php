<?php
/**
 * Footer Template - Production Ready
 * 
 * Complete footer with:
 * - All translatable strings using pll__() and esc_html()/esc_attr_e()
 * - Accessibility attributes (aria-labels, role attributes for SVG icons)
 * - Responsive Tailwind grid layout matching React design
 * - RankMath JSON-LD schema for Organization/WebSite
 * - wp_nav_menu() for Company section using 'footer_menu' location
 * - All security escaping (esc_url, esc_html, esc_attr)
 * - WordPress hooks and helper functions
 * 
 * @package Wataco
 */
...
?>
        ...
        <!-- Footer -->
        <footer class="bg-[#1A2B3C] text-white pt-12 sm:pt-16 pb-8 sm:pb-10 lg:pt-24 lg:pb-12 border-t border-white/10" role="contentinfo">
            <div class="max-w-[1440px] mx-auto px-6">
                <!-- Footer Grid - 4 Columns Responsive -->
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10 lg:gap-12 mb-12 md:mb-16 lg:mb-20">
                        ...
                        <div class="flex space-x-4">
                            ...
                            <?php if (!empty($social_links['facebook'])) : ?>
                                <a href="<?php echo esc_url($social_links['facebook']); ?>"
                                   target="_blank"
                                   rel="noopener noreferrer"
                                   aria-label="<?php echo esc_attr(pll__('Visit us on Facebook')); ?>"
                                   class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-[#228B22] transition-colors"
                                   title="<?php echo esc_attr(pll__('Visit us on Facebook')); ?>">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" role="img" aria-hidden="true">
                            ...
                            <?php if (!empty($social_links['youtube'])) : ?>
                                <a href="<?php echo esc_url($social_links['youtube']); ?>"
                                   target="_blank"
                                   rel="noopener noreferrer"
                                   aria-label="<?php echo esc_attr(pll__('Visit us on YouTube')); ?>"
                                   class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-[#228B22] transition-colors"
                                   title="<?php echo esc_attr(pll__('Visit us on YouTube')); ?>">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" role="img" aria-hidden="true">
                    ...
                    <div>
                        ...
                        <ul class="space-y-4 text-sm text-gray-400">
                            <li>
                                <a href="#" class="hover:text-[#FFD700] transition-colors">
                                    <?php echo esc_html(pll__('Web Development')); ?>
                                </a>
                            </li>
                            <li>
                                <a href="#" class="hover:text-[#FFD700] transition-colors">
                                    <?php echo esc_html(pll__('Mobile Apps')); ?>
                                </a>
                            </li>
                            <li>
                                <a href="#" class="hover:text-[#FFD700] transition-colors">
                                    <?php echo esc_html(pll__('Cloud Services')); ?>
                                </a>
                            </li>
                            <li>
                                <a href="#" class="hover:text-[#FFD700] transition-colors">
                                    <?php echo esc_html(pll__('Consulting')); ?>
                                </a>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div>
                        ...
                        wp_nav_menu(array(
                            ...
                            'container'       => false,
                            'fallback_cb'     => 'wataco_footer_menu_fallback',
                            'items_wrap'      => '<ul class="space-y-4 text-sm text-gray-400">%3$s</ul>',
                            'depth'           => 1,
                            'link_before'     => '<span class="hover:text-[#FFD700] transition-colors">',
                            'link_after'      => '</span>',
                        ));
                        ?>
                    </div>
                        ...
                        <ul class="space-y-4 text-sm text-gray-400">
                            ...
                            <?php if (!empty($contact_info['email'])) : ?>
                                <li class="flex items-center">
                                    ...
                                    <a href="<?php echo esc_url('mailto:' . antispambot($contact_info['email'])); ?>" class="hover:text-white transition-colors">
                                        <?php echo esc_html(antispambot($contact_info['email'])); ?>
                                    </a>
                                </li>
                            <?php endif; ?>
                            ...
                            <?php if (!empty($contact_info['phone'])) : ?>
                                <li class="flex items-center">
                                    ...
                                    <a href="<?php echo esc_url('tel:' . esc_attr(str_replace(array(' ', '-', '(', ')'), '', $contact_info['phone']))); ?>" class="hover:text-white transition-colors">
                ...
                <!-- Bottom Bar -->
                <div class="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center text-xs text-gray-500 space-y-4 md:space-y-0">
                    <p>
                        &copy; <?php echo esc_html(wataco_get_current_year()); ?> 
                        <?php bloginfo('name'); ?>. 
                        <?php echo esc_html(pll__('All rights reserved')); ?>
                    </p>
